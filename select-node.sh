...
# 获取当前节点信息
echo "当前节点组：节点选择"
CURRENT_NODE=$(curl -s http://127.0.0.1:9090/proxies/节点选择 | python3 -c "import json; print(json.loads(input())['now'])")
echo "当前选中节点：$CURRENT_NODE"
echo ""

# 获取可用节点列表
NODES_JSON=$(curl -s http://127.0.0.1:9090/proxies/节点选择)
...
# 使用curl命令切换节点
curl -s -X PUT -d "{\"name\": \"$SELECTED_NODE\"}" http://127.0.0.1:9090/proxies/节点选择

# 验证切换结果
sleep 1
NEW_CURRENT_NODE=$(curl -s http://127.0.0.1:9090/proxies/节点选择 | python3 -c "import json; print(json.loads(input())['now'])")
